...
describe("openAICompatible adapter", () => {
    ...
    it("generates text from chat completions response", async () => {
        const fetchMock = vi.fn().mockResolvedValue(new Response(JSON.stringify({ choices: [{ message: { content: "第一章正文" } }] }), { status: 200 }));
        vi.stubGlobal("fetch", fetchMock);
    ...
    it("reports provider generation errors", async () => {
        ...
        await expect(generateWithOpenAICompatible({ config, apiKey: "secret", prompt: "write" })).rejects.toThrow("Provider generation failed");
    });
    it("reports reasoning-only empty responses with a max token hint", async () => {
        vi.stubGlobal("fetch", vi.fn().mockResolvedValue(new Response(JSON.stringify({ choices: [{ message: { reasoning_content: "思考过程", content: "" } }] }), {
            status: 200
        })));
        await expect(generateWithOpenAICompatible({ config, apiKey: "secret", prompt: "write" })).rejects.toThrow("increase provider.maxTokens above 3000");
    });
});
