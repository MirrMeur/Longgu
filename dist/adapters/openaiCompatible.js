export async function checkOpenAICompatible(config, apiKey) {
...
export async function generateWithOpenAICompatible(request) {
    ...
    if (!text) {
        if (message?.reasoning_content) {
            throw new Error(`Provider generation failed: empty response content after reasoning output. This looks like a reasoning model exhausted its output budget; increase provider.maxTokens above ${request.config.provider.maxTokens}.`);
        }
...
function resolveRequestMaxTokens(model, configuredMaxTokens) {
...
function isLikelyReasoningModel(model) {
    ...
    return ["reasoning", "thinking", "deepseek-r", "deepseek-v4", "r1", "kimi-k2", "o1", "o3", "o4"].some((marker) => normalized.includes(marker));
}
