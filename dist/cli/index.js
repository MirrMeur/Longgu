...
import { auditChapterPlan, auditVolumePlan, createBookPlanDraft, createChaptersPlanDraft, createVolumePlanDraft } from "../core/bookPlan.js";
...
import { compareExperiment, createExperiment, diagnoseExperiment, ExperimentSortKeySchema, generateExperimentVariant, registerExperimentVariant, scoreExperimentVariant } from "../core/experiments.js";
import { recordChapterFeedback } from "../core/feedback.js";
import { exportHostBatchPrompt, exportHostChapterPrompt, importHostBatchDrafts, importHostChapterDraft, writeChapter } from "../core/generation.js";
...
model
    ...
    await runCli(async () => {
        ...
        for (const profile of listModelProfiles(config)) {
            console.log(`${profile.id}\t${profile.provider}\t${profile.model}\t${profile.apiKeyEnv}\tinput/1K=${profile.inputPer1K}\toutput/1K=${profile.outputPer1K}`);
        }
        console.log("Routes:");
...
settle
    ...
    await runCli(async () => {
        ...
        for (const diff of result.diff) {
            ...
            console.log(`${diff.ledger}: ${changed} touched, ${diff.added.length} added, ${diff.updated.length} updated, ${diff.unchanged.length} unchanged`);
        }
...
async function checkWorkspace(workspaceDir) {
...
async function loadConfigWithFriendlyErrors(workspaceDir) {
    try {
...
function parseExperimentSortKey(value) {
