...
import { createFixtureWorkspace, createHostOnlyFixtureWorkspace, createPlanningStateFixture, createRoutingFixtureWorkspace } from "../test/testUtils.js";
...
describe("longgu plan CLI", () => {
    it("creates planning drafts through chapters", async () => {
        ...
        const volumeResult = await execFileAsync(process.execPath, ["--import", "tsx", cliPath, "plan", "volume", "--id", "001", dir], { cwd: path.resolve(".") });
        const chaptersResult = await execFileAsync(process.execPath, ["--import", "tsx", cliPath, "plan", "chapters", "--volume", "001", "--skip-volume-audit", dir], { cwd: path.resolve(".") });
        ...
        await expect(readFile(path.join(dir, "outlines", "volume-001.draft.json"), "utf8")).resolves.toContain("\"schemaVersion\": \"longgu.volume-plan-draft.v0.2\"");
        ...
        await expect(readFile(path.join(dir, "outlines", "chapters-001.draft.json"), "utf8")).resolves.toContain("\"schemaVersion\": \"longgu.chapters-plan-draft.v0.2\"");
    });
    it("blocks chapter planning without a passed volume audit by default", async () => {
        ...
        await expect(execFileAsync(process.execPath, ["--import", "tsx", cliPath, "plan", "chapters", "--volume", "001", dir], {
            cwd: path.resolve(".")
...
describe("longgu plan audit CLI", () => {
    it("audits a generated volume plan draft", async () => {
        ...
        const result = await execFileAsync(process.execPath, ["--import", "tsx", cliPath, "audit", "volume-plan", "--id", "001", dir], { cwd: path.resolve(".") });
        expect(result.stdout).toContain("Volume plan audit JSON:");
        expect(result.stdout).toContain("Status: needs-revision");
        expect(result.stdout).toContain("Warning:");
        await expect(readFile(path.join(dir, "audits", "volume-001.plan-audit.json"), "utf8")).resolves.toContain("\"schemaVersion\": \"longgu.volume-plan-audit.v0.2\"");
        await expect(readFile(path.join(dir, "audits", "volume-001.plan-audit.md"), "utf8")).resolves.toContain("Volume Plan Audit 001");
    });
    it("audits a generated chapter plan draft", async () => {
        ...
        await execFileAsync(process.execPath, ["--import", "tsx", cliPath, "plan", "chapters", "--volume", "001", "--skip-volume-audit", dir], {
            cwd: path.resolve(".")
        });
        const result = await execFileAsync(process.execPath, ["--import", "tsx", cliPath, "audit", "chapter-plan", "--volume", "001", dir], { cwd: path.resolve(".") });
        expect(result.stdout).toContain("Chapter plan audit JSON:");
        expect(result.stdout).toContain("Status: needs-revision");
        expect(result.stdout).toContain("Warning:");
        await expect(readFile(path.join(dir, "audits", "chapters-001.plan-audit.json"), "utf8")).resolves.toContain("\"schemaVersion\": \"longgu.chapter-plan-audit.v0.2\"");
        await expect(readFile(path.join(dir, "audits", "chapters-001.plan-audit.md"), "utf8")).resolves.toContain("Chapter Plan Audit 001");
    });
});
...
describe("longgu feedback CLI", () => {
    it("records chapter feedback", async () => {
        ...
        await createFixtureWorkspace(dir);
        const result = await execFileAsync(process.execPath, ["--import", "tsx", cliPath, "feedback", "chapter", "--id", "001", "--score", "6", "--comment", "情节推进太慢", dir], { cwd: path.resolve(".") });
...
describe("longgu host LLM write CLI", () => {
    it("exports a host prompt and imports a host generated chapter", async () => {
        ...
        const promptResult = await execFileAsync(process.execPath, ["--import", "tsx", cliPath, "write", "chapter", "--id", "001", "--host-prompt", dir], { cwd: path.resolve(".") });
        ...
        const importResult = await execFileAsync(process.execPath, ["--import", "tsx", cliPath, "write", "chapter", "--id", "001", "--input", "drafts/001.md", dir], { cwd: path.resolve(".") });
    ...
    it("requires passed chapter-plan audit for planned host prompt unless skipped", async () => {
        ...
        await expect(execFileAsync(process.execPath, ["--import", "tsx", cliPath, "write", "chapter", "--id", "001", "--host-prompt", dir], {
            cwd: path.resolve(".")
        })).rejects.toMatchObject({
            stderr: expect.stringContaining("Chapter plan audit is required before drafting")
        });
        const skipped = await execFileAsync(process.execPath, ["--import", "tsx", cliPath, "write", "chapter", "--id", "001", "--host-prompt", "--skip-plan-audit", dir], { cwd: path.resolve(".") });
    ...
    it("supports --force as an obvious chapter-plan audit bypass", async () => {
        ...
        await createPlanningStateFixture(dir);
        await rm(path.join(dir, "audits", "chapters-001.plan-audit.json"));
        const forced = await execFileAsync(process.execPath, ["--import", "tsx", cliPath, "write", "chapter", "--id", "001", "--host-prompt", "--force", dir], { cwd: path.resolve(".") });
...
describe("longgu experiment CLI", () => {
    it("creates, registers, scores, and compares experiment variants", async () => {
        ...
        const createResult = await execFileAsync(process.execPath, ["--import", "tsx", cliPath, "experiment", "create", "--id", "opening-ab", "--goal", "测试开篇钩子", dir], { cwd: path.resolve(".") });
        ...
        const compareResult = await execFileAsync(process.execPath, ["--import", "tsx", cliPath, "experiment", "compare", "--id", "opening-ab", "--sort", "hook", dir], { cwd: path.resolve(".") });
        expect(compareResult.stdout).toContain("Compare JSON:");
        await expect(readFile(path.join(dir, "experiments", "opening-ab", "compare.json"), "utf8")).resolves.toContain("\"schemaVersion\": \"longgu.experiment-compare.v0.9\"");
        const compare = JSON.parse(await readFile(path.join(dir, "experiments", "opening-ab", "compare.json"), "utf8"));
...
describe("longgu context CLI", () => {
    it("builds a chapter context pack", async () => {
        ...
        await createPlanningStateFixture(dir);
        const result = await execFileAsync(process.execPath, ["--import", "tsx", cliPath, "context", "build", "--chapter", "001", "--max-tokens", "200", dir], { cwd: path.resolve(".") });
        ...
        await expect(readFile(path.join(dir, "context", "001.context.json"), "utf8")).resolves.toContain("\"schemaVersion\": \"longgu.context-pack.v0.7\"");
...
describe("longgu state CLI", () => {
    it("initializes baseline state ledgers", async () => {
        ...
        await expect(readFile(path.join(dir, "state", "truth.json"), "utf8")).resolves.toContain("\"schemaVersion\": \"longgu.story-state.v0.3\"");
    });
    it("inspects and settles chapter state", async () => {
        ...
        const settleResult = await execFileAsync(process.execPath, ["--import", "tsx", cliPath, "settle", "chapter", "--id", "001", "--delta", deltaPath, dir], { cwd: path.resolve(".") });
    ...
    it("checks reader promise debt with chapter context", async () => {
        ...
        await writeFile(path.join(dir, "state", "reader-promises.json"), `${JSON.stringify({
        ...
        const result = await execFileAsync(process.execPath, ["--import", "tsx", cliPath, "state", "check", "--chapter", "v1-050", "--promise-max-age", "5", dir], { cwd: path.resolve(".") });
        ...
        await expect(readFile(path.join(checksDir, markdownFile ?? ""), "utf8")).resolves.toContain("reader-promises/promise-v1-001");
    });
});
describe("longgu audit CLI", () => {
    it("audits a chapter from provided raw audit input", async () => {
        ...
        const result = await execFileAsync(process.execPath, ["--import", "tsx", cliPath, "audit", "chapter", "--id", "001", "--input", inputPath, dir], { cwd: path.resolve(".") });
        ...
        await expect(readFile(path.join(dir, "audits", "001.audit.json"), "utf8")).resolves.toContain("\"schemaVersion\": \"longgu.chapter-audit.v0.4\"");
...
describe("longgu revise CLI", () => {
    it("revises a chapter from provided Markdown input", async () => {
        ...
        await writeFile(path.join(dir, "chapters", "001.md"), "# 第一章\n\n陆沉站在门口。\n", "utf8");
        await mkdir(path.join(dir, "audits"), { recursive: true });
        await writeFile(path.join(dir, "audits", "001.audit.json"), `${JSON.stringify({
            schemaVersion: "longgu.chapter-audit.v0.4",
            chapterId: "001",
            genre: "玄幻",
        ...
        const result = await execFileAsync(process.execPath, ["--import", "tsx", cliPath, "revise", "chapter", "--id", "001", "--input", revisionInput, dir], { cwd: path.resolve(".") });
