...
import { BookPlanDraftSchema, ChapterPlanAuditSchema, ChaptersPlanDraftSchema, VolumePlanAuditSchema, VolumePlanDraftSchema, auditChapterPlan, auditVolumePlan, createBookPlanDraft, createChaptersPlanDraft, createVolumePlanDraft, loadBookPlanDraft, loadChaptersPlanDraft, loadVolumePlanDraft } from "./bookPlan.js";
import { latestRun } from "./runs.js";
async function writeVolumePlanAuditFixture(workspaceDir, overrides = {}) {
...
describe("createBookPlanDraft", () => {
    it("creates a validated book draft from config and bible context", async () => {
        ...
        expect(result.draft.sourceDigest.find((item) => item.file === "bible/premise.md")?.excerpt).toContain("少年负债入宗门");
...
describe("createVolumePlanDraft", () => {
    ...
    it("requires a book draft before volume planning", async () => {
        const dir = await mkdtemp(path.join(os.tmpdir(), "longgu-plan-volume-"));
        await createFixtureWorkspace(dir);
        await expect(createVolumePlanDraft({ workspaceDir: dir, volumeId: "001" })).rejects.toThrow("Run longgu plan book");
    });
    ...
    it("rejects unsafe volume ids", async () => {
        ...
        await createFixtureWorkspace(dir);
        await createBookPlanDraft({ workspaceDir: dir });
        await expect(createVolumePlanDraft({ workspaceDir: dir, volumeId: "../001" })).rejects.toThrow("Plan id must contain");
    });
});
describe("createChaptersPlanDraft", () => {
    ...
    it("requires a volume draft before chapter planning", async () => {
        const dir = await mkdtemp(path.join(os.tmpdir(), "longgu-plan-chapters-"));
        await createFixtureWorkspace(dir);
        await expect(createChaptersPlanDraft({ workspaceDir: dir, volumeId: "001" })).rejects.toThrow("Run longgu plan volume --id 001");
    });
    it("requires a passed volume audit before chapter planning", async () => {
        ...
        await expect(createChaptersPlanDraft({ workspaceDir: dir, volumeId: "001" })).rejects.toThrow("Volume plan audit is required before chapter planning for volume 001");
    });
    it("rejects failed volume audits before chapter planning", async () => {
        ...
        await expect(createChaptersPlanDraft({ workspaceDir: dir, volumeId: "001" })).rejects.toThrow("Review audits/volume-001.plan-audit.md");
    });
    ...
    it("rejects unsafe volume ids", async () => {
        const dir = await mkdtemp(path.join(os.tmpdir(), "longgu-plan-chapters-"));
        await createFixtureWorkspace(dir);
        await expect(createChaptersPlanDraft({ workspaceDir: dir, volumeId: "../001" })).rejects.toThrow("Plan id must contain");
    });
});
describe("auditVolumePlan", () => {
    it("passes a ready volume plan and writes audit artifacts", async () => {
        ...
        expect(VolumePlanAuditSchema.parse(JSON.parse(await readFile(result.jsonPath, "utf8"))).schemaVersion).toBe("longgu.volume-plan-audit.v0.2");
    ...
    it("blocks structurally invalid volume plans", async () => {
        ...
        await createBookPlanDraft({ workspaceDir: dir });
        const planResult = await createVolumePlanDraft({ workspaceDir: dir, volumeId: "001" });
        await writeFile(planResult.outputPath, `${JSON.stringify({ ...planResult.draft, conflictEscalation: [], chapterSeedCount: 0 }, null, 2)}\n`, "utf8");
        const result = await auditVolumePlan({
            workspaceDir: dir,
            volumeId: "001",
            now: new Date("2026-06-10T01:00:00.000Z")
    ...
    it("requires a volume plan before auditing", async () => {
        ...
        await createFixtureWorkspace(dir);
        await expect(auditVolumePlan({ workspaceDir: dir, volumeId: "001" })).rejects.toThrow("Volume plan draft is required before audit");
    });
});
describe("auditChapterPlan", () => {
    it("passes a ready chapter plan and writes audit artifacts", async () => {
        ...
        expect(result.audit.blocked).toBe(false);
        expect(result.audit.issues).toEqual([]);
        expect(ChapterPlanAuditSchema.parse(JSON.parse(await readFile(result.jsonPath, "utf8"))).schemaVersion).toBe("longgu.chapter-plan-audit.v0.2");
    ...
    it("blocks chapter count mismatches", async () => {
        ...
        await writeVolumePlanAuditFixture(dir);
        const planResult = await createChaptersPlanDraft({ workspaceDir: dir, volumeId: "001" });
        await writeFile(planResult.outputPath, `${JSON.stringify({ ...planResult.draft, chapterCount: planResult.draft.chapterCount + 1 }, null, 2)}\n`, "utf8");
        const result = await auditChapterPlan({
            workspaceDir: dir,
            volumeId: "001",
            now: new Date("2026-06-09T06:00:00.000Z")
    ...
    it("requires a chapter plan before auditing", async () => {
        ...
        await createFixtureWorkspace(dir);
        await expect(auditChapterPlan({ workspaceDir: dir, volumeId: "001" })).rejects.toThrow("Chapters plan draft is required before audit");
    });
});
