...
describe("chapter revision", () => {
    ...
    it("revises a chapter and writes revision history", async () => {
        ...
        await expect(readFile(path.join(result.revisionDir, "metadata.json"), "utf8")).resolves.toContain("\"schemaVersion\": \"longgu.chapter-revision.v0.5\"");
        const run = await latestRun(dir);
        expect(run?.metadata.task).toBe("revise");
        const context = JSON.parse(await readFile(path.join(run?.dir ?? "", "context.json"), "utf8"));
...
async function writeAudit(dir, audit) {
