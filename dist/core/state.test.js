...
import { HooksLedgerSchema, ReaderPromisesLedgerSchema, ResourcesLedgerSchema, StateDeltaSchema, TimelineLedgerSchema, TruthLedgerSchema, CharactersLedgerSchema, checkState, initStateLedgers, inspectState, loadStateLedger, settleChapterState, stateLedgerFiles } from "./state.js";
...
describe("checkState", () => {
    ...
    it("reports dangling resource owner references", async () => {
        ...
        await writeFile(path.join(dir, "state", "resources.json"), `${JSON.stringify(ResourcesLedgerSchema.parse({
            schemaVersion: "longgu.story-state.v0.3",
            ledger: "resources",
            updatedAt: "2026-06-09T09:00:00.000Z",
            resources: [
    ...
    it("reports character role drift from settled facts and timeline events", async () => {
        ...
        await writeFile(path.join(dir, "state", "characters.json"), `${JSON.stringify(CharactersLedgerSchema.parse({
            schemaVersion: "longgu.story-state.v0.3",
            ledger: "characters",
            updatedAt: "2026-06-09T09:00:00.000Z",
            characters: [
        ...
        await writeFile(path.join(dir, "state", "truth.json"), `${JSON.stringify(TruthLedgerSchema.parse({
            schemaVersion: "longgu.story-state.v0.3",
            ledger: "truth",
            updatedAt: "2026-06-09T09:00:00.000Z",
    ...
    it("reports timeline order and repeated first-time scene drift", async () => {
        ...
        await writeFile(path.join(dir, "state", "timeline.json"), `${JSON.stringify(TimelineLedgerSchema.parse({
            schemaVersion: "longgu.story-state.v0.3",
            ledger: "timeline",
            updatedAt: "2026-06-09T09:00:00.000Z",
            events: [
        ...
        expect(result.report.status).toBe("needs-review");
        expect(result.report.issues.map((issue) => issue.id)).toEqual(expect.arrayContaining([
    ...
    it("reports overdue active reader promises when chapter context is provided", async () => {
        ...
        await writeFile(path.join(dir, "state", "reader-promises.json"), `${JSON.stringify(ReaderPromisesLedgerSchema.parse({
            schemaVersion: "longgu.story-state.v0.3",
...
describe("settleChapterState", () => {
    it("applies a valid state delta and writes a settlement record", async () => {
        ...
        await writeFile(deltaPath, `${JSON.stringify(StateDeltaSchema.parse({
        ...
        await expect(readFile(path.join(result.settlementDir, "metadata.json"), "utf8")).resolves.toContain("\"schemaVersion\": \"longgu.state-settlement.v0.3\"");
    ...
    it("retries model extraction once after invalid output", async () => {
        ...
        expect(calls).toBe(2);
        await expect(readFile(path.join(result.settlementDir, "model-attempts.json"), "utf8")).resolves.toContain("\"accepted\": false");
        expect(TruthLedgerSchema.parse(await loadStateLedger(dir, "truth.json")).facts).toHaveLength(1);
    });
    it("fails model settlement after retry without mutating ledgers", async () => {
        ...
        let calls = 0;
        await expect(settleChapterState({
            workspaceDir: dir,
            chapterId: "001",
            config: {
                title: "测试小说",
