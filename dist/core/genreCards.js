...
const genericCard = GenreCardSchema.parse({
    id: "generic",
    name: "通用",
    aliases: ["generic", "通用"],
    generic: true,
...
export function listGenreCards() {
    return [...genreCards];
}
export function resolveGenreCard(genre) {
    const normalized = normalizeGenre(genre);
    return (genreCards.find((card) => normalizeGenre(card.id) === normalized || card.aliases.some((alias) => normalizeGenre(alias) === normalized)) ?? genericCard);
}
export function renderGenrePromptHints(card) {
