...
import { applyTokenBudget, buildChapterContext, ContextPackSchema } from "./context.js";
describe("chapter context builder", () => {
    it("builds reviewable JSON and Markdown context artifacts", async () => {
        ...
        expect(result.pack.sections.map((section) => section.id)).toEqual(expect.arrayContaining([
            "chapter-card",
            "volume-plan",
            "genre-card",
            "style-constraints",
        ...
        const writtenJson = ContextPackSchema.parse(JSON.parse(await readFile(result.jsonPath, "utf8")));
    ...
    it("includes human feedback in context packs", async () => {
        ...
        await writeFile(path.join(dir, "feedback", "001.feedback.json"), `${JSON.stringify({
