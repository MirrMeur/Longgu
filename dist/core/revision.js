...
export async function reviseChapter(input) {
    ...
    if (preCriticalCount > 0 && postCriticalCount !== undefined && postCriticalCount >= preCriticalCount) {
        throw new Error(`Chapter revision failed: post-audit critical count did not decrease (${preCriticalCount} -> ${postCriticalCount}).`);
    }
    const revisedAt = input.now ?? new Date();
    const revisionDir = path.join(input.workspaceDir, "revisions", input.chapterId, revisedAt.toISOString().replace(/[:.]/g, "-"));
...
async function loadChapterAudit(auditPath) {
    const raw = await readFile(auditPath, "utf8");
    return ChapterAuditSchema.parse(JSON.parse(raw));
}
...
function selectRevisionIssues(audit) {
...
function countCritical(audit) {
    return audit.issues.filter((issue) => issue.severity === "critical").length;
}
function normalizeRevisedMarkdown(text) {
