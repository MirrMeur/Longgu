...
export const BookPlanDraftSchema = z.object({
    ...
    retentionRisks: z.array(z.object({
        risk: z.string(),
        mitigation: z.string()
    })),
    sourceFiles: z.array(z.string().min(1)).min(1),
    sourceDigest: z.array(z.object({
        file: z.string().min(1),
        excerpt: z.string()
    })),
...
export const VolumePlanDraftSchema = z.object({
    ...
    conflictEscalation: z.array(z.object({
        step: z.string(),
        pressure: z.string(),
...
export async function createVolumePlanDraft(input) {
    ...
    const exists = await pathExists(outputPath);
    if (exists && !input.force) {
        throw new Error(`Volume plan draft already exists. Re-run with --force to replace outlines/volume-${volumeId}.draft.json.`);
    }
...
export async function createChaptersPlanDraft(input) {
    ...
    const exists = await pathExists(outputPath);
    if (exists && !input.force) {
        throw new Error(`Chapters plan draft already exists. Re-run with --force to replace outlines/chapters-${volumeId}.draft.json.`);
    }
...
async function writeGuidedChapterPlanFiles(workspaceDir, draft, overwrite) {
...
function renderGuidedChapterPlan(chapter) {
...
async function assertVolumePlanAuditPassed(workspaceDir, volumeId) {
    ...
    if (!(await pathExists(auditPath))) {
        throw new Error(`Volume plan audit is required before chapter planning for volume ${volumeId}. Run longgu audit volume-plan --id ${volumeId}, or pass --skip-volume-audit.`);
    }
    const raw = await readFile(auditPath, "utf8");
    const audit = VolumePlanAuditSchema.parse(JSON.parse(raw));
    if (audit.volumeId !== volumeId || audit.status !== "passed" || audit.blocked) {
        throw new Error(`Volume plan audit has not passed for volume ${volumeId}. Review audits/volume-${volumeId}.plan-audit.md, fix outlines/volume-${volumeId}.draft.json, rerun longgu audit volume-plan --id ${volumeId}, or pass --skip-volume-audit.`);
    }
}
...
function contextToPromptContext(sourceFiles, context) {
    return sourceFiles.map((file) => ({
        file,
        content: file === "longgu.yaml" ? "loaded from config" : context.find((item) => item.file === file)?.content ?? ""
    }));
}
function createChapterCards(volumePlan, targetWords) {
...
function scaffoldBookPlan(seed, context) {
    ...
    return BookPlanDraftSchema.parse({
        ...
        retentionRisks: seed.retentionRisks.length > 0
            ? seed.retentionRisks
            : [{ risk: "设定解释过多导致推进变慢。", mitigation: "把规则说明压进冲突、选择和爽点兑现里。" }]
    });
}
function scaffoldVolumePlan(seed, bookPlan) {
    ...
    return VolumePlanDraftSchema.parse({
        ...
        resourceChanges: seed.resourceChanges.length > 0
...
function contextText(context, basename) {
...
function firstChineseName(text) {
    const match = text.match(/[\u4e00-\u9fa5]{2,4}/u);
    return match?.[0] ?? "";
}
function collectChapterPlanIssues(plan) {
    const issues = [];
    if (plan.chapterCount !== plan.chapters.length) {
        issues.push(ChapterPlanAuditIssueSchema.parse({
...
function collectVolumePlanIssues(plan) {
    const issues = [];
    for (const field of volumeReadinessFields) {
        const value = plan[field];
        if (isWeakPlanField(value)) {
            issues.push(VolumePlanAuditIssueSchema.parse({
                id: `${field}-weak`,
                severity: "warning",
                field,
...
const volumeReadinessFields = ["volumeGoal", "primaryAntagonist", "endingHook"];
const chapterReadinessFields = ["goal", "conflict", "payoff", "informationGain", "endingHook"];
const repeatedChapterFields = ["goal", "payoff", "endingHook"];
...
function isWeakChapterPlanField(value) {
    return isWeakPlanField(value);
}
function isWeakPlanField(value) {
...
function isWeakPlanLabel(value) {
    const normalized = normalizePlanText(value);
...
function renderVolumePlanAuditMarkdown(audit) {
    const issues = audit.issues.length
        ? audit.issues
            .map((issue) => `- [${issue.severity}] ${issue.id}${issue.step ? ` (${issue.step}/${issue.field})` : ` (${issue.field})`}\n  - Reason: ${issue.reason}\n  - Fix: ${issue.fix}`)
            .join("\n")
        : "- No issues.";
...
function renderChapterPlanAuditMarkdown(audit) {
    const issues = audit.issues.length
        ? audit.issues
            .map((issue) => `- [${issue.severity}] ${issue.id}${issue.chapterId ? ` (${issue.chapterId}/${issue.field})` : ` (${issue.field})`}\n  - Reason: ${issue.reason}\n  - Fix: ${issue.fix}`)
            .join("\n")
        : "- No issues.";
...
function resolveConflictStage(volumePlan, index) {
...
export async function loadBookPlanDraft(filePath) {
    const raw = await readFile(filePath, "utf8");
    return BookPlanDraftSchema.parse(JSON.parse(raw));
}
export async function loadVolumePlanDraft(filePath) {
    const raw = await readFile(filePath, "utf8");
    return VolumePlanDraftSchema.parse(JSON.parse(raw));
}
async function loadVolumePlanAuditInput(filePath) {
    const raw = await readFile(filePath, "utf8");
    return VolumePlanAuditInputSchema.parse(JSON.parse(raw));
}
export async function loadChaptersPlanDraft(filePath) {
    const raw = await readFile(filePath, "utf8");
    return ChaptersPlanDraftSchema.parse(JSON.parse(raw));
}
...
function pickLabeledLine(context, fileName, labels) {
