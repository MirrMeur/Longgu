...
const chapterContractFields = ["startHook", "protagonistGoal", "obstacle", "turn", "payoff", "tailHook"];
const chapterContractFieldLabels = {
    startHook: "开头压力/钩子",
...
function normalizeChapterContract(raw) {
    const explicitMissing = raw?.missing ?? [];
    const values = Object.fromEntries(chapterContractFields.map((field) => [field, normalizeContractText(raw?.[field])]));
    const missing = chapterContractFields.filter((field) => explicitMissing.includes(field) || isMissingContractValue(values[field]));
    ...
    return ChapterContractSchema.parse({
        ...
        diagnosis: normalizeContractText(raw?.diagnosis) ||
...
export function normalizeCheckerPriority(priority) {
    switch (priority) {
        case "P0":
            return "critical";
        case "P1":
...
async function loadRawAudit(inputPath) {
    const raw = await readFile(inputPath, "utf8");
    return { raw: RawChapterAuditSchema.parse(JSON.parse(raw)) };
}
...
async function loadRunContextFiles(workspaceDir, files) {
...
async function resolveChapterBody(workspaceDir, chapterId) {
    const candidates = [
...
async function loadGuidedAuditSnapshot(workspaceDir, chapterId) {
    const files = [
        "已确认决定.md",
...
async function loadStateSnapshot(workspaceDir) {
    const files = [];
    const ledgers = {};
...
function renderAuditPrompt(context) {
...
function renderAuditRetryPrompt(input) {
...
function parseRawAuditFromText(text) {
    return RawChapterAuditSchema.parse(parseProviderJsonObject(text, "Chapter audit extraction failed: provider response did not contain a JSON object."));
}
async function loadPayoffRecipes(workspaceDir) {
...
function renderAuditMarketText(config) {
    if (!config.market) {
        return "";
    }
    const platformRules = {
...
async function writeGuidedAuditMarkdown(workspaceDir, audit) {
...
function renderGuidedAuditMarkdown(audit) {
...
function renderAuditMarkdown(audit) {
    const issues = audit.issues.length
        ? audit.issues
            .map((issue) => `- [${issue.severity}] ${issue.id} (${issue.source}/${issue.dimension})\n  - Location: ${issue.location}\n  - Reason: ${issue.reason}\n  - Fix: ${issue.fix}`)
            .join("\n")
        : "- No issues.";
