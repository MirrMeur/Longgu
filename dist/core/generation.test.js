...
describe("writeChapter", () => {
    ...
    it("renders drafting prompt from the context pack", async () => {
        ...
        const chapterPlan = JSON.parse(await readFile(chapterPlanPath, "utf8"));
        ...
        const run = await latestRun(dir);
        expect(run?.metadata.inputFiles).toEqual(expect.arrayContaining(["outlines/chapters-001.draft.json", "chapters/000.md"]));
    });
    ...
    it("blocks drafting when chapter-plan audit has not passed", async () => {
        ...
        await createPlanningStateFixture(dir);
        await writeFile(path.join(dir, "audits", "chapters-001.plan-audit.json"), `${JSON.stringify({
    ...
    it("applies the chapter-plan audit gate to host prompt export", async () => {
        ...
        await createPlanningStateFixture(dir);
        await rm(path.join(dir, "audits", "chapters-001.plan-audit.json"));
        await expect(exportHostChapterPrompt({ workspaceDir: dir, chapterId: "001" })).rejects.toThrow("Chapter plan audit is required before drafting");
    });
    it("imports a host LLM draft and records zero-cost host metadata", async () => {
        ...
        await createPlanningStateFixture(dir);
        await writeFile(path.join(dir, "longgu.yaml"), `title: 测试小说
genre: 玄幻
language: zh-CN
context:
  maxTokens: 16000
`, "utf8");
    ...
    it("applies the chapter-plan audit gate to host draft import", async () => {
        ...
        await mkdir(path.join(dir, "drafts"), { recursive: true });
        await writeFile(path.join(dir, "drafts", "001.md"), "# 宿主标题\n\n不应导入。", "utf8");
        await expect(importHostChapterDraft({
            workspaceDir: dir,
            chapterId: "001",
            inputPath: "drafts/001.md"
        })).rejects.toThrow("Chapter plan audit is required before drafting");
...
async function writeCompoundChapterPlan(root, volumeId, chapters) {
    await mkdir(path.join(root, "outlines"), { recursive: true });
    await writeFile(path.join(root, "outlines", `chapters-${volumeId}.draft.json`), `${JSON.stringify({
    ...
    await writeFile(path.join(root, "audits", `chapters-${volumeId}.plan-audit.json`), `${JSON.stringify({
        schemaVersion: "longgu.chapter-plan-audit.v0.2",
        volumeId,
        status: "passed",
        blocked: false,
