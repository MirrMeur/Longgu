...
import { compareExperiment, createExperiment, diagnoseExperiment, ExperimentCompareSchema, generateExperimentVariant, registerExperimentVariant, scoreExperimentVariant } from "./experiments.js";
...
describe("experiments", () => {
    it("creates, registers, scores, and compares variants", async () => {
        ...
        const written = ExperimentCompareSchema.parse(JSON.parse(await readFile(compared.jsonPath, "utf8")));
    ...
    it("sorts variants by audited chapter contract strength", async () => {
        ...
        await writeFile(path.join(dir, "audits", "incomplete.audit.json"), `${JSON.stringify(createAudit("incomplete"), null, 2)}\n`, "utf8");
...
function createAudit(contractStatus) {
    ...
    return {
        ...
        issues: contractStatus === "complete"
            ? []
            : [
                {
                    id: "contract-tail",
