...
export async function analyzePacing(input) {
    ...
    const payoffInterval = payoffChapters.length < 2
        ? null
...
export function analyzeChapterPacing(chapterId, text, characterNames = []) {
...
async function resolveChapterRange(workspaceDir, from, to) {
...
async function loadCharacterNames(workspaceDir) {
...
function countCpScenes(text, characterNames) {
...
function detectFatigueRisk(chapters) {
    const risks = [];
...
function renderPacingMarkdown(report) {
    const rows = report.chapters
        .map((chapter) => `| ${chapter.chapterId} | ${chapter.wordCount} | ${chapter.emotionalIntensity} | ${chapter.hasCliffhanger ? "yes" : "no"} | ${chapter.payoffCueCount} | ${chapter.cpScreentime} |`)
        .join("\n");
