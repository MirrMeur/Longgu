...
import { z } from "zod";
import { ChaptersPlanDraftSchema, VolumePlanDraftSchema } from "./bookPlan.js";
...
export async function buildChapterContext(input) {
    ...
    const sections = applyTokenBudget(candidates.map((candidate) => ({
        ...candidate,
...
export function applyTokenBudget(sections, tokenBudget) {
    const next = sections.map((section) => ({ ...section }));
    const removablePriorities = ["low", "medium", "high"];
    for (const priority of removablePriorities) {
        const candidates = next
            .map((section, index) => ({ section, index }))
            .filter(({ section }) => section.included && section.priority === priority)
            .sort((left, right) => contextRetentionScore(left.section) - contextRetentionScore(right.section) ||
...
function contextRetentionScore(section) {
    let score = 0;
...
async function collectContextCandidates(workspaceDir, chapterId) {
...
async function findChapterPlan(workspaceDir, chapterId) {
...
async function findGuidedChapterPlan(workspaceDir, chapterId) {
...
async function findVolumePlan(workspaceDir, volumeId) {
    ...
    return { file: relative, plan: VolumePlanDraftSchema.parse(JSON.parse(raw)) };
}
async function loadChapterSummaries(workspaceDir, chapterId) {
    ...
    for (const file of entries.filter((entry) => entry.endsWith(".summary.json")).sort()) {
        ...
        const raw = await readFile(path.join(workspaceDir, relative), "utf8");
        const summary = ChapterSummarySchema.parse(JSON.parse(raw));
...
async function loadBibleSections(workspaceDir) {
    ...
    return bibleContext
        ...
        source: item.file,
        reason: item.file === path.join("bible", "payoff-recipes.md")
...
async function loadGuidedCreativeSections(workspaceDir, guidedContext) {
    const candidates = [];
    const files = [
...
async function loadPreviousChapterBodies(workspaceDir, chapterId) {
...
function splitNaturalParts(input) {
...
async function loadFeedbackSections(workspaceDir, chapterId) {
...
async function loadStateSections(workspaceDir) {
    const sections = [];
...
function renderStateLedgerSummary(ledger) {
...
function stateLedgerCounts(ledger) {
...
function stateLedgerFocus(ledger) {
    switch (ledger.ledger) {
        case "truth":
...
async function loadStyleSection(workspaceDir) {
...
function renderHumanBrief(pack) {
    const section = (id) => pack.sections.find((item) => item.id === id && item.included)?.content.trim() ?? "";
...
function renderMarketConstraints(market) {
    const platformRules = {
...
function extractEndingHook(chapterCard) {
    const match = chapterCard.match(/"endingHook"\s*:\s*"([^"]+)"/);
    return match?.[1] ?? "";
}
function includedTokens(sections) {
