...
export const GUIDED_ROOT_FILES = ["当前步骤.md", "创作流程.md", "已确认决定.md", "项目说明.md"];
...
export const GUIDED_SKILLS = ["longgu-start", "longgu-plan", "longgu-write", "longgu-review", "longgu-state", "longgu-help"];
const guidedSkillFiles = Object.fromEntries(GUIDED_SKILLS.map((skill) => [`.claude/skills/${skill}/SKILL.md`, renderLongguSkill(skill)]));
const starterFiles = {
...
export async function assertWorkspaceShape(workspaceDir) {
    const missing = [];
...
export async function assertGuidedWorkspaceShape(workspaceDir) {
    const missing = [];
    for (const relativePath of [
        ...GUIDED_ROOT_FILES,
        ...GUIDED_DIRECTORIES,
...
export async function loadBibleContext(workspaceDir) {
...
export function renderChapterPlanTemplate(chapterId) {
...
export function renderAiReviewTemplate(chapterId) {
...
function renderLongguSkill(skill) {
    const descriptions = {
        "longgu-start": {
            title: "Longgu Start",
