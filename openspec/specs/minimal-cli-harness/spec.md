...
The system SHALL initialize a Longgu novel workspace with the minimum file and directory structure required for V0.1 writing.

#### Scenario: Initialize a new novel workspace
- **WHEN** a user runs `longgu init` in an empty target directory
- **THEN** the system creates `longgu.yaml`, `bible/`, `chapters/`, and `runs/`
...
- **WHEN** a user runs `longgu init` in a directory that already contains V0.1 workspace files
